...
export const MultiDateInput: React.FC<MultiDateInputProps> = ({ label, value, onChange, placeholder, required, error }) => {
    return (
        <Paper
            p="md"
            radius="20px"
        ...
        >
            <Stack gap="sm">
                ...
                <MantineDatePickerInput
                    ...
                    value={(value || []).map(d => dayjs(d).toDate())}
                    onChange={(dates) => onChange(dates.map(d => dayjs(d).format("YYYY-MM-DD")))}
                    error={error}
                    size="md"
                    radius="12px"
                    clearable
